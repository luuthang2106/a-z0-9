...
export default function Posts({ posts }) {
    ...
    if (posts.length > 0) {
        postRows = posts.map((post, index) => (
            <tr className="w-full font-light text-gray-700 bg-gray-100 whitespace-no-wrap border border-b-0" key={index}>
                ...
                <td className="px-4 py-4">{post.slug}</td>
                <td className="px-4 py-4">
                    {
                        post.isPublished ? <span className="text-sm bg-green-500 text-white rounded-full px-2 py-1">Active</span> : <span className="text-sm bg-red-500 text-white rounded-full px-2 py-1">Inactive</span>
                    }
                </td>
                <td className="px-4 py-4">
                    ...
                    <a className="inline-block cursor-pointer" onClick={() => deletePost(post.slug)}>
    ...
    async function deletePost(slug) {
        const response = await window.fetch(`/api/a/post/${slug}`, { method: 'DELETE' })
        ...
        <div className="bg-white rounded-lg shadow-lg py-6 w-full h-full">
            <div className="py-3 mx-6">
                <Link href="/admin/post/new">
                    <button type="button" className="bg-blue-600 text-gray-200 text-lg rounded hover:bg-blue-500 px-6 py-3 focus:outline-none">Add New Post</button>
                </Link>
            </div>
